 ...
 */
			...
			uTemp &= uMask;
			printBin("Utemp is: ", uTemp, SUBKEYSIZE);
			printf("****\n");
				for(; i< SUBKEYSIZE; i++){
					printf("i = %d, bit checked = %d\n",i,(7-(SUBKEYSIZE - p8[i] - 1)));
					// if (1 << (KEYSIZE - p8[i] - 1) & uTemp){
					if (1 << (SUBKEYSIZE - p8[i] - 1) & uTemp){
					
						uResult |= 1 << (KEYSIZE - i - 3);
						// printf("%d %d ---> %d\n",i, KEYSIZE - p8[i] - 1, KEYSIZE -i-3);
						printf("bit changed = %d\n", 7-(KEYSIZE - i - 3));
					}
				}
				printBin("uResult: ", uResult, SUBKEYSIZE);
				return uResult;
			}
/**
 * Key Schedule
 */
			void keySchedule(UINT key10,UINT key8[]){
				...
				printBin("After PC1: ",keyTemp, SUBKEYSIZE+2);
				splitKey(keyTemp, key5);
				printBin("Post split left: ",key5[0], SUBKEYSIZE - 3);
				printBin("Post split right: ", key5[1], SUBKEYSIZE - 3);
				for(i=0; i<2 ; i++){
					key5[0] = leftShift(key5[0], i+1, SPLITKEYSIZE);
					printBin("Post first shift: ",key5[0], SUBKEYSIZE - 3);
					key5[1] = leftShift(key5[1], i+1, SPLITKEYSIZE);
					printBin("Post first shift: ", key5[1], SUBKEYSIZE - 3);
					key8[i]=box_p8(key5);
					//key8[i] = per(p8, key5);
					printBin("key is:", key8[i], SUBKEYSIZE);
 ...
 */
				BYTE f(BYTE bRight, BYTE key){
					...
					while(--i >= 0)
						if( 01 << (4 - E[SUBKEYSIZE - i - 1] - 1) & bRight )
							bRes |= (01 << i);
						printBin("After E:", bRes, SUBKEYSIZE);
						bRes ^= key;
						printBin("After key xoring:", bRes, SUBKEYSIZE);
						split824(bRes,sLR4);
						c = (sLR4[0] & 6) >> 1;
						...
						bTemp = sLR4[0] | sLR4[1];
						printBin("SBOX op:", bTemp, SUBKEYSIZE/2);
						bRes = 00;
 // permute using P4
						i=4;
						while(--i >= 0)
							if( 01 << (4 - P4[4 - i - 1] - 1) & bTemp )
								bRes |= (01 << i); 
							printBin("Final return :", bRes, SUBKEYSIZE/2);
							return bRes;
						}
						int main(void){

							UINT key8[2]={0,0};
							UINT key10 = cbin2UINT("0000000001",KEYSIZE);
							BYTE input8 = (BYTE) cbin2UINT("10100101",BLOCKSIZE), exInput8, i;
 /** left and Right */
							BYTE LR[] ={00,00};
							// char useKey;
							char userKey[]="0101110011",userText[]="10000001";
 ...
 /** display the input */
							...
							printBin("key1 is :", key8[0], SUBKEYSIZE);
							printBin("key2 is :", key8[1], SUBKEYSIZE);
							putchar('\n');
							printf(" =========================== Encrypt ============================ \n");
							input8 = per(IP,input8);
// ===> Start of the round
							printBin("After 1st permute:", input8, SUBKEYSIZE);
							exInput8 = input8;
							printBin("b4 exInput8: ",exInput8, 8);
							for(i=0; i< 2; i++){
 /** =====> begin round */
 ...
 printBin("Op from SBOX: ",input8, SUBKEYSIZE);
 printBin("Other part: ", LR[1], SUBKEYSIZE);
 input8 |= LR[1];
 exInput8 = ((input8 & 240) >> 4) | ( (input8 & 15) << 4 );
 printBin("exInput8: ",exInput8, 8);
 /** =====> end of round */
}
printBin("Before IP: ", exInput8, SUBKEYSIZE);

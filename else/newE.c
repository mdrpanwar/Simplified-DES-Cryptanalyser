...
while(--i >= 0)
	...
	// 		printf("S0 IP pair %d xor",gc);
	// 		printBin(" :",SBIP[0]^SBXIP[0],BLOCKSIZE);
	// 		printf("S1 IP pair %d xor",gc);
	// 		printBin(" :",SBIP[1]^SBXIP[1],BLOCKSIZE);
	// 		if (((SBIP[0]^SBXIP[0])==dex) && ((SBIP[1]^SBXIP[1])==dex2)){//int compared with unsigned char (BYTE)
	// 			// printf("#######\n");
	// 			// printf("dex = %d, xor at S0 = ",dex,(SBIP[0]^SBXIP[0]));
	// 			// printf("dex2 = %d, xor at S1 = ",dex2,(SBIP[1]^SBXIP[1]));
	// 			// printBin("S0",SBIP[0]^SBXIP[0],BLOCKSIZE);
	// 			// printBin("S1",SBIP[1]^SBXIP[1],BLOCKSIZE);
	// 			// printf("#######\n");
	// 			matchSimulSB++;
	// 		}		
	// 		printf("\n");
...
int main(void){
	...
	extendDC();
	printBin("R1XCHAR ",R1XCHAR,BLOCKSIZE);
	printBin("R1YCHAR ",R1YCHAR,BLOCKSIZE);
	scanf("%c",&cont);
	for(i=0;i<255;i++)
		PK2[i]=0;
	printf("Press enter to continue\n");
	...
	for(input=0;input<255;input++){
		gc=input;
		run=1;crypt(input);g=1;
		curR1Y=R1Y;
		run=1;crypt((BYTE)(input^R1XCHAR));g=0;
		// printBin("the xor ",R1Y^curR1Y,BLOCKSIZE);
	...
	for(i=0;i<255;i++)
		printf("Key %d = %d \n",i,PK2[i]);
	printf("Simultaneous SBoxes input diff. matches=%d\n",matchSimulSB);
	if(matchSimulSB==0){
		printf("CONFLICT, count must be zero.\n"); //based on my observation that the conflict bw dex and dex2 happens iff matchSimulSB = 0
	}
	else{
		printf("NON-CONFLICT\n");
	}
...
/*possibly do this for both the two variants of S1, and two variants of E, i.e. a total of 4 runs of the following type
*[extendDC() needs to be changed for our E]
his S1, his E
his S1, our E
our S1, his E
our S1, our E
For all of these four runs, note the summary of results as described below.
########################################
run for all possible (dex,dey) and (dex2,dey2)
noting:
whether it's a conflict // test by using E
matchSimulSB
count
whether key was guessed right or not
whether count = max(PK2)  in case the count is non-zero



this will tell us:
is it so that whenever count is non-zero, it guessess correct key. can use this to just get the first non-zero count and hence the key
[a global variable that stores how many times count is non-zero and another to store how many times the key was guessed correctly]

whether the following are true:

matchSimulSB is either 0 or 255
[global array storing the unique vaues of matchSimulSB]

conflict bw dex and dex2 happens iff matchSimulSB = 0
[an array storing ordered pairs of (dex,dex2) type whenever conflict is there,
another array to store the ordered pairs when matchSimulSB is zero,
do these arrays match exatly]
